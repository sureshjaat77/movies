        ...
        try: 
            ...
            if refData and refData.split("-", 1)[0] == "Jisshu":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        ...
                        reply_markup = InlineKeyboardMarkup(buttons)
                        m=await message.reply_sticker("CAACAgUAAxkBAAEOEAdn0SHJYgytuH-VZDm-1Bn1ESBX1wACKhIAAs_VKFRhdY8poes42jYE") 
...
async def save_template(client, message):
    ...
    await message.reply_text(
        f"Successfully updated the template for {title}:\n\n"
        f"**Template:**\n{template}\n\n"
        f"**Image URL:**\n{image_url}\n\n"
        f"**Buttons:**\n{len(buttons)} added",
        disable_web_page_preview=True
...
# Default shortener settings
SHORTENER_WEBSITE = "omegalinks.in"
SHORTENER_API = "abf4946daf36969475b795fa16a3e1613db3f9bb"
SHORTENER_WEBSITE2 = "omegalinks.in"
SHORTENER_API2 =  "abf4946daf36969475b795fa16a3e1613db3f9bb"
SHORTENER_WEBSITE3 = "omegalinks.in"
SHORTENER_API3 =  "abf4946daf36969475b795fa16a3e1613db3f9bb"
...
async def set_shortner(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner omegalinks.in abf4946daf36969475b795fa16a3e1613db3f9bb`</b>")
        return        
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/suresh_jaat_7').json()
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nᴅᴇꜰᴀᴜʟᴛ - {SHORTENER_WEBSITE}\nᴀᴘɪ - `{SHORTENER_API}`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)

# Similarly, update for set_verify_2 and set_verify_3
# Replace SHORTENER_WEBSITE2, SHORTENER_API2 and SHORTENER_WEBSITE3, SHORTENER_API3 where needed.
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner_2  omegalinks.in abf4946daf36969475b795fa16a3e1613db3f9bb`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
...
async def set_shortner_3(c, m):
    ...
    if len(m.text.split()) == 1:
        return await m.reply("<b>Use this command like this - \n\n`/set_shortner_3 omegalinks.in abf4946daf36969475b795fa16a3e1613db3f9bb`</b>")
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await sts.delete()
    ...
    if not await is_check_admin(c, grp_id, userid):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.command) == 1:
        await m.reply_text("<b>ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀᴅᴅ sʜᴏʀᴛɴᴇʀ & ᴀᴘɪ\n\nᴇx - `/set_shortner_3 omegalinks.in abf4946daf36969475b795fa16a3e1613db3f9bb`</b>", quote=True)
        return
    try:
        URL = m.command[1]
        API = m.command[2]
